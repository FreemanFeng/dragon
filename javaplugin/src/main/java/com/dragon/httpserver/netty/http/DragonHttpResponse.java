...
package com.dragon.httpserver.netty.http;
...
public class DragonHttpResponse extends DefaultFullHttpResponse {
    ...
    private DragonHttpResponse(HttpResponseStatus status, ByteBuf buffer ) {
    ...
    public static FullHttpResponse make(HttpResponseStatus status) {
        if (HttpResponseStatus.UNAUTHORIZED == status) {
            return DragonHttpResponse.make(HttpResponseStatus.UNAUTHORIZED, CONTENT_ERROR_401);
        }
        if (HttpResponseStatus.NOT_FOUND == status) {
            return DragonHttpResponse.make(HttpResponseStatus.NOT_FOUND, CONTENT_ERROR_404);
        }
        if (HttpResponseStatus.METHOD_NOT_ALLOWED == status) {
            return DragonHttpResponse.make(HttpResponseStatus.METHOD_NOT_ALLOWED, CONTENT_ERROR_405);
        }
        return DragonHttpResponse.make(HttpResponseStatus.OK,CONTENT_NORMAL_200);
    }
    ...
    public static FullHttpResponse makeError(Exception exception) {
        ...
        return DragonHttpResponse.make(HttpResponseStatus.INTERNAL_SERVER_ERROR, String.format(CONTENT_ERROR_500,message));
    }
    ...
    private static FullHttpResponse make(HttpResponseStatus status,String content) {
        ...
        DragonHttpResponse response = new DragonHttpResponse(status,buffer);
